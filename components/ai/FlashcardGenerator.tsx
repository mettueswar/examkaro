...
import { Layers, Plus, Trash2, Play, Loader2, Sparkles, BookOpen, ChevronDown } from 'lucide-react';
...
export function FlashcardGenerator({ onStudy, showDecksOnly = false }: Props) {
  ...
  useEffect(() => { fetchDecks(); fetchMaterials(); }, [fetchDecks, fetchMaterials]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setShowForm(false);
      setForm({ materialId: '', customText: '', title: '', count: 20, language: 'english' });
      fetchDecks();
    } catch (err: unknown) {
    ...
    <div>
          ...
          {showForm && (
            <div className="card p-5 mb-5 border-2 border-violet-200 bg-violet-50/30">
              <h4 className="font-semibold text-surface-800 mb-4 flex items-center gap-2">
                <Sparkles size={15} className="text-violet-600" /> AI Flashcard Generator
              </h4>
              <div className="grid sm:grid-cols-2 gap-4 mb-4">
                <div className="sm:col-span-2">
                  <label className="block text-xs font-semibold text-surface-700 mb-1">Deck Title *</label>
                  <input
                    value={form.title}
                ...
                <div>
                  ...
                  <div className="relative">
                    <select
                      ...
                      onChange={e => setForm(p => ({ ...p, materialId: e.target.value, customText: '' }))}
                      className="input-base text-sm appearance-none pr-8"
                    >
                      <option value="">— Use custom text below —</option>
                      {materials.map(m => (
                        <option key={m.id} value={m.id}>{m.title} ({m.wordCount?.toLocaleString()} words)</option>
                      ))}
                    </select>
                ...
                <div>
                  <label className="block text-xs font-semibold text-surface-700 mb-1">Number of Cards</label>
                  <input
                    type="number" min={5} max={50}
                    value={form.count}
                    onChange={e => setForm(p => ({ ...p, count: parseInt(e.target.value) || 20 }))}
                    className="input-base text-sm"
                  />
                </div>
                <div>
                ...
                {!form.materialId && (
                  <div className="sm:col-span-2">
                    <label className="block text-xs font-semibold text-surface-700 mb-1">Custom Text (if no material selected)</label>
                    <textarea
                      value={form.customText}
                      onChange={e => setForm(p => ({ ...p, customText: e.target.value }))}
                      rows={4}
                      className="input-base text-sm resize-none"
                      placeholder="Paste study content here... (minimum 100 characters)"
                    />
                    <p className="text-xs text-surface-400 mt-1">{form.customText.length} chars</p>
                  </div>
                )}
              </div>
              ...
              {generating && (
                <p className="text-xs text-violet-600 mt-3 animate-pulse">
                  ✨ Gemini AI is creating your flashcards... This may take 15-30 seconds.
                </p>
              )}
            </div>
          )}
      ...
      {loading ? (
        ...
        <div className="card p-12 text-center text-surface-400">
          ...
          <p className="text-sm mt-1">Generate your first deck from any study material</p>
        </div>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {decks.map(deck => (
            <div key={deck.id} className="card p-5 hover:shadow-elevated transition-all group">
              <div className="flex items-start justify-between gap-2 mb-3">
                <div className="w-10 h-10 bg-violet-100 rounded-xl flex items-center justify-center">
                  <Layers size={18} className="text-violet-600" />
                </div>
                <button onClick={() => deleteDeck(deck.id)} className="p-1.5 text-surface-300 hover:text-red-500 hover:bg-red-50 rounded-lg opacity-0 group-hover:opacity-100 transition-all">
                  <Trash2 size={13} />
                </button>
              </div>
              <h4 className="font-bold text-surface-800 text-sm mb-1 line-clamp-2">{deck.title}</h4>
              <div className="flex items-center gap-2 text-xs text-surface-400 mb-4">
                <span>{deck.cardCount} cards</span>
                <span>·</span>
                <span className="capitalize">{deck.language}</span>
                <span>·</span>
                <span>{deck.studyCount} reviews</span>
              </div>
              <button
