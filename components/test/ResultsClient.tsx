...
import {
  CheckCircle, XCircle, MinusCircle, Clock, Trophy,
  BarChart2, BookOpen, ChevronDown, ChevronUp, ArrowRight, RotateCcw,
} from 'lucide-react';
...
export function ResultsClient({ attempt, test, questions }: Props) {
  ...
  const totalMarks = attempt.totalMarks ?? test.totalMarks;
  ...
  const filtered = questionResults.filter((r) => filter === 'all' || r.status === filter);

  const summaryCards = [
    {
      label: 'Score', value: `${score.toFixed(1)}/${totalMarks}`,
      icon: Trophy, color: 'text-brand-600', bg: 'bg-brand-50',
    },
    {
      label: 'Correct', value: attempt.correct ?? 0,
      icon: CheckCircle, color: 'text-success-600', bg: 'bg-success-50',
    },
    {
      label: 'Incorrect', value: attempt.incorrect ?? 0,
      icon: XCircle, color: 'text-danger-600', bg: 'bg-danger-50',
    },
    {
      label: 'Skipped', value: attempt.skipped ?? 0,
      icon: MinusCircle, color: 'text-surface-500', bg: 'bg-surface-100',
    },
    {
      label: 'Time Taken', value: formatTime(timeTaken),
      icon: Clock, color: 'text-warning-500', bg: 'bg-warning-50',
    },
    {
      label: 'Accuracy', value: `${attempt.correct && questions.length ? Math.round(((attempt.correct) / questions.length) * 100) : 0}%`,
        ...
        <div className="flex flex-col sm:flex-row items-center gap-6">
          <div className="relative w-28 h-28 shrink-0">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className={cn('text-2xl font-display font-bold', getScoreColor(percentage))}>
                {percentage}%
              </span>
          ...
          <div className="text-center sm:text-left">
            ...
            <div className="flex items-center gap-2 mt-3">
              <Link href={`/tests/${test.slug}`} className="flex items-center gap-1.5 text-sm btn-outline py-1.5 px-3">
                <RotateCcw size={13} /> Reattempt
              </Link>
              <Link href="/tests" className="flex items-center gap-1.5 text-sm btn-primary py-1.5 px-3">
      ...
      {tab === 'summary' ? (
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div className="card p-5">
            <h3 className="font-semibold text-surface-800 text-sm mb-4">Performance Breakdown</h3>
            {[
              { label: 'Correct', count: attempt.correct ?? 0, color: 'bg-success-500', total: questions.length },
              { label: 'Incorrect', count: attempt.incorrect ?? 0, color: 'bg-danger-500', total: questions.length },
              { label: 'Skipped', count: attempt.skipped ?? 0, color: 'bg-surface-300', total: questions.length },
          ...
          {/* Time Stats */}
          <div className="card p-5">
            <h3 className="font-semibold text-surface-800 text-sm mb-4">Time Statistics</h3>
            <div className="space-y-3">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold">
                  {questions.length > 0 ? Math.round(timeTaken / questions.length) : 0}s
                </span>
              </div>
            </div>
          </div>
        ...
        <div>
          ...
          <div className="space-y-3">
            {filtered.map(({ question: q, ans, status, correctOption }, idx) => (
              <div key={q.id} className={cn(
                'card overflow-hidden border-l-4',
                status === 'correct' ? 'border-l-success-500' :
                status === 'incorrect' ? 'border-l-danger-500' : 'border-l-surface-300'
              )}>
                <button
                ...
                >
                  <div className="flex items-center gap-3">
                    {status === 'correct' ? <CheckCircle size={16} className="text-success-500 shrink-0" /> :
                     status === 'incorrect' ? <XCircle size={16} className="text-danger-500 shrink-0" /> :
                     <MinusCircle size={16} className="text-surface-400 shrink-0" />}
                    <span className="text-sm font-medium text-surface-700">Q{idx + 1}.</span>
                    <span
                      className="text-sm text-surface-700 line-clamp-1"
                      dangerouslySetInnerHTML={{ __html: q.text }}
                    />
                  </div>
                  {expandedQ === q.id ? <ChevronUp size={15} className="text-surface-400 shrink-0" /> : <ChevronDown size={15} className="text-surface-400 shrink-0" />}
                </button>
                      ...
                      {q.options.map((opt) => {
                        ...
                        return (
                          <div key={opt.id} className={cn(
                            ...
                            isSelected ? 'bg-danger-50 border-danger-200 text-danger-700' :
                            'bg-surface-50 border-surface-100 text-surface-600'
                          )}>
                            <span className={cn(
                              'w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold shrink-0',
                              isCorrect ? 'bg-success-500 text-white' :
                              isSelected ? 'bg-danger-500 text-white' : 'bg-surface-300 text-surface-600'
                            )}>
                              {opt.id}
                            </span>
                            <span dangerouslySetInnerHTML={{ __html: opt.text }} />
                            {isCorrect && <span className="ml-auto text-xs font-medium text-success-600">✓ Correct</span>}
                            {isSelected && !isCorrect && <span className="ml-auto text-xs font-medium text-danger-600">✗ Your Answer</span>}
                          </div>
                        );
                      })}
                    </div>
