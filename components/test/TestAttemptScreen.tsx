...
import {
  ...
  CheckCircle, AlertCircle, Clock, Send, X,
  Maximize2, Minimize2, Languages, ChevronDown,
  Loader2, User, BookmarkPlus,
} from 'lucide-react';
...
// ─── Status colors matching reference image ───────────────────────────────────
const STATUS_STYLES: Record<QuestionStatus, { bg: string; label: string }> = {
  not_visited:     { bg: 'bg-surface-300 text-surface-600 border-surface-300',     label: 'Not Visited' },
  not_answered:    { bg: 'bg-red-500 text-white border-red-500',                    label: 'Not Answered' },
  answered:        { bg: 'bg-green-500 text-white border-green-500',                label: 'Answered' },
  marked:          { bg: 'bg-purple-500 text-white border-purple-500',              label: 'Marked' },
  answered_marked: { bg: 'bg-purple-400 text-white border-purple-400',              label: 'Answered & Marked' },
};

export function TestAttemptScreen({ attemptId }: Props) {
  const { user } = useAuth();
  const {
    data, loading, answers, currentIndex, timeLeft,
    submitting, stats, selectOption, clearAnswer,
    toggleMark, navigateTo, submit,
  ...
  const currentQ = filteredQuestions[currentFilteredIndex >= 0 ? currentFilteredIndex : 0] ?? allQuestions[currentIndex];

  // ─── Live Translation ────────────────────────────────────────────────────────
  const doTranslate = useCallback(async (q: typeof currentQ, lang: 'english' | 'hindi') => {
    if (!q || lang === 'english') { setTranslatedQ(''); setTranslatedOpts({}); return; }
    // If test has Hindi text already, use it
    if (q.textHindi) { setTranslatedQ(q.textHindi); return; }
    setTranslating(true);
    try {
      const res = await fetch('/api/translate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ text: q.text.replace(/<[^>]+>/g, ''), from: 'en', to: 'hi' }),
      });
      ...
      await Promise.all(q.options.map(async (opt) => {
        if (opt.textHindi) { optResults[opt.id] = opt.textHindi; return; }
    ...
    } catch { /* silent fail, show original */ }
    finally { setTranslating(false); }
  }, []);

  // Translate when question or language changes
  useEffect(() => {
    clearTimeout(translateTimer.current);
    if (language === 'hindi') {
      translateTimer.current = setTimeout(() => doTranslate(currentQ, 'hindi'), 400);
    } else {
      setTranslatedQ('');
      setTranslatedOpts({});
    }
  ...
  const { test } = data;
  const qText = language === 'hindi' && translatedQ ? translatedQ : currentQ?.text ?? '';
  ...
  return (
    <div className={cn('flex flex-col bg-slate-100', fullscreen ? 'fixed inset-0 z-50' : 'min-h-screen')}>
      ...
      <header className="bg-blue-700 text-white px-4 py-2 flex items-center justify-between shrink-0 shadow-md">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <p className="text-sm font-bold leading-none hidden sm:block">Online Assessment Platform</p>
        ...
        <main className="flex-1 flex flex-col overflow-hidden bg-white border-r border-slate-200">
          ...
          <div className="border-b border-slate-200 bg-slate-50 px-4 py-2 flex items-center gap-3 flex-wrap shrink-0">
            ...
            <div className="flex items-center gap-1.5 bg-white border border-slate-200 rounded-lg p-1">
              {(['english', 'hindi'] as const).map(lang => (
                ...
                >
                  ...
                  {lang === 'hindi' && translating && <Loader2 size={10} className="animate-spin" />}
                </button>
              ))}
            </div>
            ...
            <span className="text-xs text-slate-400 hidden sm:block">
              View in: <strong className="text-slate-600">{language === 'hindi' ? 'HINDI' : 'ENGLISH'}</strong>
            </span>
          </div>
          ...
          <div className="px-5 pt-4 pb-2 flex items-center gap-3 shrink-0">
            ...
            <span className="badge bg-blue-50 text-blue-600 text-xs">+{currentQ?.marks ?? 1} Mark</span>
            {(currentQ?.negativeMarks ?? 0) > 0 && (
              <span className="badge bg-red-50 text-red-600 text-xs">-{currentQ?.negativeMarks} Wrong</span>
            )}
          ...
          <div className="flex-1 overflow-y-auto px-5 py-2">
            ...
            {currentQ?.image && (
              <div className="mb-4">
                <img src={currentQ.image} alt="Question" className="max-h-48 rounded-lg border border-slate-200 object-contain" />
              </div>
            )}
            ...
            <div className="space-y-3 pb-4">
              {options.map((opt) => {
                const optText = language === 'hindi' && translatedOpts[opt.id]
          ...
          <div className="border-t border-slate-200 bg-white px-5 py-3 flex items-center justify-between gap-2 shrink-0">
            <div className="flex items-center gap-2">
              <button
                onClick={() => {
                  ...
                  const nextIdx = (globalCurrentIdx >= 0 ? globalCurrentIdx : 0) + 1;
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => {
                  const prev = (globalCurrentIdx >= 0 ? globalCurrentIdx : 0) - 1;
              ...
              {globalCurrentIdx < allQuestions.length - 1 ? (
                <button
                  onClick={() => navigateTo((globalCurrentIdx >= 0 ? globalCurrentIdx : 0) + 1)}
        ...
        <aside className="hidden lg:flex flex-col w-72 bg-slate-50 border-l border-slate-200 shrink-0 overflow-hidden">
          ...
          <div className="bg-white border-b border-slate-200 p-4">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="font-bold text-slate-800 text-sm truncate">{user?.name || 'Candidate'}</p>
            ...
            <div className="grid grid-cols-4 gap-1 mt-3">
              {[
                { label: 'Done', count: stats.answered, color: 'text-green-600 bg-green-50' },
                { label: 'Skip', count: stats.notAnswered, color: 'text-red-600 bg-red-50' },
                { label: 'Mark', count: stats.marked, color: 'text-purple-600 bg-purple-50' },
                { label: 'New', count: stats.notVisited, color: 'text-slate-500 bg-slate-100' },
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isCurrent && 'ring-2 ring-offset-1 ring-blue-500 scale-105 shadow-md'
                    )}
                  >
                    {idx + 1}
                    {/* Marked indicator dot */}
                    {answers[q.id]?.markedForReview && status === 'answered_marked' && (
          ...
          <div className="border-t border-slate-200 bg-white px-4 py-3">
            ...
            <div className="grid grid-cols-2 gap-x-2 gap-y-1.5">
              {[
                ...
                { status: 'not_answered' as QuestionStatus, label: 'Not Answered' },
                { status: 'marked' as QuestionStatus,       label: 'Marked' },
                { status: 'not_visited' as QuestionStatus,  label: 'Not Visited' },
              ].map(({ status, label }) => (
                <div key={status} className="flex items-center gap-1.5">
                  <div className={cn('w-5 h-5 rounded flex items-center justify-center text-xs font-bold border-2 shrink-0', STATUS_STYLES[status].bg)}>
                    {stats.answered}
                  </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            <div className="text-center mb-5">
              ...
              <p className="text-sm text-slate-500 mt-1">You cannot change answers after submitting.</p>
            </div>

            <div className="grid grid-cols-3 gap-3 mb-5">
              {[
                { label: 'Answered', count: stats.answered, color: 'bg-green-50 text-green-700 border-green-200' },
                { label: 'Skipped',  count: stats.notAnswered + stats.notVisited, color: 'bg-red-50 text-red-700 border-red-200' },
                { label: 'Marked',   count: stats.marked,   color: 'bg-purple-50 text-purple-700 border-purple-200' },
              ].map(({ label, count, color }) => (
            ...
            <div className="flex gap-3">
              <button onClick={() => setConfirmOpen(false)} disabled={submitting} className="flex-1 border border-slate-200 text-slate-700 font-semibold py-2.5 rounded-xl hover:bg-slate-50 transition-all">
                Review More
              </button>
              <button
...
function MobilePalette({
  ...
  return (
    <>
      ...
      >
        <span className="text-xs font-bold leading-tight text-center">Q<br />List</span>
      </button>
          ...
          <div className="relative bg-white rounded-t-2xl w-full max-h-[80vh] flex flex-col shadow-2xl">
            ...
            <div className="grid grid-cols-4 gap-2 p-4 border-b border-slate-100">
              {[
                ...
                { label: 'Not Ans.', count: stats.notAnswered, cls: 'bg-red-50 text-red-700' },
                { label: 'Marked', count: stats.marked, cls: 'bg-purple-50 text-purple-700' },
                { label: 'Unvisited', count: stats.notVisited, cls: 'bg-slate-100 text-slate-600' },
              ].map(({ label, count, cls }) => (
                <div key={label} className={cn('rounded-xl p-2 text-center', cls)}>
              ...
              <div className="grid grid-cols-7 gap-2">
                {filteredQuestions.map((q, idx) => {
                  ...
                  return (
                    <button
                      key={q.id}
                      onClick={() => { navigateTo(globalIdx); setOpen(false); }}
                      className={cn(
                        'w-full aspect-square rounded-lg text-xs font-bold border-2 transition-all',
                        STATUS_STYLES[status].bg,
                        q.id === currentQ?.id && 'ring-2 ring-offset-1 ring-blue-500 scale-110'
                      )}
                    >
                      {idx + 1}
                    </button>
