...
import { extractYouTubeTranscript, extractWebsiteContent, extractPDFContent, extractDOCXContent, cleanText, wordCount } from '@/lib/ai/extractor';
import { z } from 'zod';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!extractedText || extractedText.length < 100) {
      return errorResponse('Could not extract enough content (minimum 100 characters)');
    }

    const result = await execute(
      `INSERT INTO study_materials (user_id, title, type, source_url, content, word_count, status, metadata)
       VALUES (?, ?, ?, ?, ?, ?, 'ready', ?)`,
      [auth.userId, materialTitle, type, url || null, extractedText, wordCount(extractedText), JSON.stringify(metadata)]
    );
    ...
    return successResponse({
      ...
      type,
    }, 'Material processed successfully', 201);
  } catch (err: unknown) {
    console.error('Material processing error:', err);
    return errorResponse(err instanceof Error ? err.message : 'Processing failed');
  }
}
...
export async function DELETE(req: NextRequest) {
  ...
  const material = await queryOne('SELECT user_id FROM study_materials WHERE id = ?', [id]);
