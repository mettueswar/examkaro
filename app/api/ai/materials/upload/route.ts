...
import { checkSuperAccess, superRequiredResponse, logAIUsage } from '@/lib/ai/guard';
import { successResponse, errorResponse } from '@/lib/security';
import { extractPDFContent, extractDOCXContent, cleanText, wordCount } from '@/lib/ai/extractor';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (file.size > MAX_SIZE) return errorResponse('File too large. Maximum 20MB');
    ...
    if (!extractedText || extractedText.length < 100) {
      return errorResponse('Could not extract readable text from file. Ensure the file is not image-only.');
    }
    ...
    const materialTitle = title || file.name.replace(/\.(pdf|docx)$/i, '') || 'Study Material';

    const result = await execute(
      `INSERT INTO study_materials (user_id, title, type, file_path, content, word_count, status, metadata)
       VALUES (?, ?, ?, ?, ?, ?, 'ready', ?)`,
      [
        auth.userId,
        materialTitle,
    ...
    }, 'File processed successfully', 201);
  } catch (err: unknown) {
    console.error('File upload error:', err);
    return errorResponse(err instanceof Error ? err.message : 'File processing failed');
  }
}
