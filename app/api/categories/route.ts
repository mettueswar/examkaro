  ...
  try {
    ...
    const result = await execute(
      `INSERT INTO categories (name, name_hindi, slug, description, icon, color, parent_id, order_index)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [data.name, data.nameHindi || null, slug, data.description || null,
       data.icon || null, data.color || null, data.parentId || null, data.orderIndex]
    );

    const category = await queryOne<Category>('SELECT * FROM categories WHERE id = ?', [result.insertId]);
  ...
  try {
    ...
    await execute(
      `UPDATE categories SET
       ...
       WHERE id = ?`,
      [data.name, data.nameHindi, data.description, data.icon, data.color, data.orderIndex, id]
    );

    return successResponse(await queryOne<Category>('SELECT * FROM categories WHERE id = ?', [id]));
  } catch (err) {
