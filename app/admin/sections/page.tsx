  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-display font-bold text-surface-900">Test Sections</h1>
        <button
          disabled={!selectedTest}
          onClick={() => setEditing({ name: '', orderIndex: sections.length })}
      ...
      {editing !== null && (
        <div className="card p-5 mb-5 border-2 border-brand-200">
          <h3 className="font-semibold text-surface-800 mb-4 text-sm">{editing.id ? 'Edit Section' : 'New Section'}</h3>
      ...
      {!selectedTest ? (
        <div className="card p-12 text-center text-surface-400">Select a test to manage its sections</div>
      ) : sections.length === 0 ? (
        <div className="card p-12 text-center">
          <p className="text-surface-500 font-medium mb-1">No sections defined</p>
          <p className="text-sm text-surface-400 mb-4">Sections are optional. Add them to organize questions into groups.</p>
          <button onClick={() => setEditing({ name: '', orderIndex: 0 })} className="btn-primary text-sm">Add First Section</button>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-surface-50 border-b border-surface-100">
                {['Order', 'Section Name', 'Hindi Name', 'Questions', 'Time Limit', 'Actions'].map(h => (
